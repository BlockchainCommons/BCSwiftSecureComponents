...
public struct SealedMessage: Sendable {
    ...
    public init(plaintext: DataProvider, recipient: PublicKeyBase, aad: Data? = nil, testKeyMaterial: DataProvider? = nil, testNonce: Nonce? = nil) {
        ...
        let sharedKey = ephemeralSender.x25519AgreementPrivateKey.sharedKey(with: recipientPublicKey)
        self.message = sharedKey.encrypt(plaintext: plaintext, aad: aad, nonce: testNonce)
        self.ephemeralPublicKey = ephemeralSender.x25519AgreementPrivateKey.publicKey
    }
    ...
    public func decrypt(with privateKeys: PrivateKeyBase) throws -> Data {
        let sharedKey = privateKeys.x25519AgreementPrivateKey.sharedKey(with: ephemeralPublicKey)
